...
function processEvent(event, calendarTz){
  //------------------------ Skip manually deleted events ------------------------
  if (skipManuallyDeletedEvents && deletedEventIds.length > 0) {
    var eventUid = event.getFirstPropertyValue('uid').toString();
    var checkId = eventUid;
    if (event.hasProperty('recurrence-id')) {
      checkId = eventUid + "_" + event.getFirstPropertyValue('recurrence-id').toString();
    }
    if (deletedEventIds.indexOf(checkId) > -1) {
      Logger.log("Skipping manually deleted event: " + (event.getFirstPropertyValue('summary') || checkId));
      return;
    }
  }
...
/**
 * Saves an array to PropertiesService, splitting into chunks if needed.
 *
 * @param {string} baseKey - The base property key name
 * @param {Array} arr - The array to save
 */
function saveArrayProperty(baseKey, arr) {
  ...
  for (var i = 0; i < chunks.length; i++) {
    props.setProperty(baseKey + "_chunk_" + i, chunks[i]);
  }
  props.setProperty(baseKey + "_chunkCount", chunks.length.toString());
  // Clean up old excess chunks
  for (var i = chunks.length; i < chunks.length + 5; i++) {
    props.deleteProperty(baseKey + "_chunk_" + i);
  }
}

/**
 * Loads an array from PropertiesService, reassembling from chunks if needed.
 *
 * @param {string} baseKey - The base property key name
 * @return {Array} The loaded array, or empty array if not found
 */
function loadArrayProperty(baseKey) {
  var props = PropertiesService.getScriptProperties();
  var countStr = props.getProperty(baseKey + "_chunkCount");
  if (countStr == null) return [];
  var count = parseInt(countStr);
  var json = "";
  for (var i = 0; i < count; i++) {
    var chunk = props.getProperty(baseKey + "_chunk_" + i);
