...
var sourceCalendars = [                // The ics/ical urls that you want to get events from along with their target calendars (list a new row for each mapping of ICS url to Google Calendar)
  ...
  ["https://outlook.office365.com/owa/calendar/4a87f89653a0459dbb0070425e6d8532@meta.com/d4d65bd7c95c4baca7b5ba812946b7f13340955564136900954/calendar.ics", "Abhi Work Calendar"],
...
var skipManuallyDeletedEvents = true;     // If true, events manually deleted from Google Calendar will not be re-synced
...
function startSync(){
  ...
  for (var calendar of sourceCalendars){
    ...
    //------------------------ Detect manually deleted events ------------------------
    if (skipManuallyDeletedEvents) {
      var propKeySuffix = targetCalendarName.replace(/[^a-zA-Z0-9]/g, "_");
      var prevKnownKey = "previouslyKnownIds_" + propKeySuffix;
      var deletedKey = "deletedEventIds_" + propKeySuffix;

      var previouslyKnownIds = loadArrayProperty(prevKnownKey);
      deletedEventIds = loadArrayProperty(deletedKey);

      // UID was in calendar last run + missing now + still in ICS feed = user deleted it
      for (var i = 0; i < previouslyKnownIds.length; i++) {
        var uid = previouslyKnownIds[i];
        if (calendarEventsIds.indexOf(uid) === -1
            && icsEventsIds.indexOf(uid) > -1
            && deletedEventIds.indexOf(uid) === -1) {
          Logger.log("Detected manually deleted event: " + uid);
          deletedEventIds.push(uid);
        }
      }

      // Remove UIDs no longer in ICS feed (natural cleanup)
      deletedEventIds = deletedEventIds.filter(function(uid) {
        return icsEventsIds.indexOf(uid) > -1;
      });

      saveArrayProperty(prevKnownKey, calendarEventsIds.filter(function(id) { return id != null; }));
      saveArrayProperty(deletedKey, deletedEventIds);

      if (deletedEventIds.length > 0) {
        Logger.log("Skipping " + deletedEventIds.length + " manually deleted events");
      }
    }
...
function clearDeletedEvents() {
  ...
  for (var key in allProps) {
    if (key.indexOf("deletedEventIds_") === 0 || key.indexOf("previouslyKnownIds_") === 0) {
      props.deleteProperty(key);
    }
  }
  Logger.log("Cleared stored deletion data. Manually deleted events will be re-synced on next run.");
}
